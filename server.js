...
app.post('/api/pressels', authenticateJwt, async (req, res) => {
    ...
    if (!name || !bot_id || !white_page_url || !Array.isArray(pixel_ids) || pixel_ids.length === 0) return res.status(400).json({ message: 'Todos os campos são obrigatórios e ao menos um pixel deve ser selecionado.' });
    try {
...
app.post('/api/settings/pushinpay', authenticateJwt, async (req, res) => {
    ...
    try { await sql`UPDATE sellers SET pushinpay_token = ${token} WHERE id = ${req.user.id}`; res.status(200).json({ message: 'Token salvo com sucesso.' });
...
app.post('/api/pix/generate', async (req, res) => {
    const apiKey = req.headers['x-api-key']; const { click_id, value_cents } = req.body;
    ...
    } catch (error) { res.status(500).json({ message: 'Erro ao gerar cobrança PIX.' }); }
});
app.post('/api/pix/check-status', async (req, res) => {
    ...
    try {
        ...
        if (paidTransaction) { return res.status(200).json({ status: 'paid', value: paidTransaction.pix_value, all_transactions: transactions }); }
        else { return res.status(200).json({ status: 'pending', all_transactions: transactions }); }
    ...
    try {
        ...
        for (const { pixel_config_id } of presselPixels) {
            ...
            if (pixelConfig.length > 0) {
                const { pixel_id, meta_api_token } = pixelConfig[0]; const event_id = `pix.${pixData.id}.${pixel_id}`;
                const payload = { data: [{
                    event_name: 'Purchase', event_time: Math.floor(Date.now() / 1000), event_id: event_id,
...
// ## NOVA ROTA PARA O DASHBOARD ANALÍTICO ##
app.get('/api/dashboard/analytics', authenticateJwt, async (req, res) => {
    try {
        ...
        const topBotQuery = sql`
            ...
            WHERE p.seller_id = ${sellerId} AND pt.status = 'paid'
            GROUP BY p.bot_name
