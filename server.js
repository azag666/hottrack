...
const app = express();
// Aumenta o limite do corpo da requisição para aceitar ficheiros em base64
...
async function processFlow(chatId, botId, botToken, sellerId, startNodeId = null, initialVariables = {}, flowData = null) {
    ...
    try {
        ...
        console.error("[Flow Engine] Erro ao carregar os dados do fluxo:", e);
        return;
    }
        ...
        switch (currentNode.type) {
            ...
            case 'action_pix': {
                try {
                    ...
                    const response = await axios.post('https://novaapi-one.vercel.app/api/pix/generate', 
            ...
            // ... (restantes cases como action_check_pix, action_city, forward_flow)
        }
        safetyLock++;
    }
}
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        res.status(500).json({ status: 'error', message: 'API com erro de conexão ao BD.', error: error.message });
    }
});
...
// ... (Restante das rotas de API para bots, flows, chat, etc. - inalteradas)
...
app.post('/api/media/upload', authenticateJwt, async (req, res) => {
    ...
    try {
        ...
        const blob = new Blob([buffer], { type: fileType === 'image' ? 'image/jpeg' : 'video/mp4' });
        ...
        const response = await sendTelegramRequest(storageBotToken, telegramMethod, formData, { 'Content-Type': `multipart/form-data; boundary=${formData._boundary}` });
...
app.post('/api/webhook/telegram/:botId', async (req, res) => {
    // ... (O seu webhook completo, como nas versões anteriores)
});
