...
async function processFlow(chatId, botId, botToken, sellerId, startNodeId = null, initialVariables = {}) {
    ...
    // Recupera o estado salvo, se houver
        ...
        switch (currentNode.type) {
            ...
            case 'action_pix': {
                try {
                    ...
                    // A geração do PIX agora é uma chamada de API para o serviço HotTrack
            ...
            case 'forward_flow': {
                ...
                if (targetFlow) {
                    // Reseta o estado para começar o novo fluxo
                    await sql`DELETE FROM user_flow_states WHERE chat_id = ${chatId} AND bot_id = ${botId}`;
                    
                    // Inicia o novo fluxo a partir do seu gatilho
                    ...
                    if (startNode) {
                        ...
                        // Chamada recursiva para processar o novo fluxo
    ...
    try {
        ...
        if (pendingTimeouts.length > 0) {
            ...
            for (const timeout of pendingTimeouts) {
                ...
                const [userState] = await sql`SELECT waiting_for_input FROM user_flow_states WHERE chat_id = ${timeout.chat_id} AND bot_id = ${timeout.bot_id}`;
                
                if (userState && userState.waiting_for_input) {
                    const [bot] = await sql`SELECT seller_id, bot_token FROM telegram_bots WHERE id = ${timeout.bot_id}`;
                    if (bot) {
                        console.log(`[CRON] Processando timeout para ${timeout.chat_id} no nó ${timeout.target_node_id}`);
                        processFlow(timeout.chat_id, bot.id, bot.bot_token, bot.seller_id, timeout.target_node_id, timeout.variables);
                    }
                }
            }
        }
...
// --- ROTAS DE LOGIN E CADASTRO (NECESSÁRIAS PARA O HOTBOT FUNCIONAR) ---
...
app.post('/api/bots', authenticateJwt, async (req, res) => {
    ...
    try {
        const placeholderToken = `placeholder_${uuidv4()}`; // Token único para evitar erro de constraint
        const [newBot] = await sql`
            INSERT INTO telegram_bots (seller_id, bot_name, bot_token) 
            VALUES (${req.user.id}, ${bot_name}, ${placeholderToken}) RETURNING *;`;
...
app.put('/api/bots/:id', authenticateJwt, async (req, res) => {
    ...
    try {
        await sql`UPDATE telegram_bots SET bot_token = ${bot_token.trim()} WHERE id = ${req.params.id} AND seller_id = ${req.user.id}`;
...
app.get('/api/flows', authenticateJwt, async (req, res) => {
    try {
        const flows = await sql`SELECT * FROM flows WHERE seller_id = ${req.user.id} ORDER BY created_at DESC`;
...
app.post('/api/flows', authenticateJwt, async (req, res) => {
    ...
    try {
        ...
        const [newFlow] = await sql`
            INSERT INTO flows (seller_id, bot_id, name, nodes) VALUES (${req.user.id}, ${botId}, ${name}, ${JSON.stringify(initialFlow)}) RETURNING *;`;
...
app.put('/api/flows/:id', authenticateJwt, async (req, res) => {
    ...
    try {
        const [updated] = await sql`UPDATE flows SET name = ${name}, nodes = ${nodes}, updated_at = NOW() WHERE id = ${req.params.id} AND seller_id = ${req.user.id} RETURNING *;`;
...
app.get('/api/chats/:botId/:chatId', authenticateJwt, async (req, res) => {
    try {
        const messages = await sql`
            SELECT * FROM telegram_chats WHERE bot_id = ${req.params.botId} AND chat_id = ${req.params.chatId} AND seller_id = ${req.user.id} ORDER BY created_at ASC;`;
...
app.post('/api/chats/:botId/send-message', authenticateJwt, async (req, res) => {
    ...
    try {
        const [bot] = await sql`SELECT bot_token FROM telegram_bots WHERE id = ${req.params.botId} AND seller_id = ${req.user.id}`;
...
app.delete('/api/chats/:botId/:chatId', authenticateJwt, async (req, res) => {
    try {
        await sql`DELETE FROM telegram_chats WHERE bot_id = ${req.params.botId} AND chat_id = ${req.params.chatId} AND seller_id = ${req.user.id}`;
        await sql`DELETE FROM user_flow_states WHERE bot_id = ${req.params.botId} AND chat_id = ${req.params.chatId}`;
        res.status(204).send();
...
app.post('/api/bots/mass-send', authenticateJwt, async (req, res) => {
    ...
    try {
        const bots = await sql`SELECT id, bot_token FROM telegram_bots WHERE id = ANY(${botIds}) AND seller_id = ${req.user.id}`;
        if (bots.length === 0) return res.status(404).json({ message: 'Nenhum bot válido selecionado.' });
        
        const users = await sql`SELECT DISTINCT ON (chat_id) chat_id, bot_id FROM telegram_chats WHERE bot_id = ANY(${botIds}) AND seller_id = ${req.user.id}`;
